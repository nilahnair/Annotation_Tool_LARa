...
class ManualAnnotationControllerVideo(Controller):
    def __init__(self, gui):
        super(ManualAnnotationControllerVideo, self).__init__(gui)
        self.current_window = -1
        self.setup_widgets()

    def setup_widgets(self):
        self.load_tab(f'..{sep}ui{sep}manual_annotation_mode_v.ui', "Manual Annotation")
    ...
    def enable_widgets(self):
        if self.enabled is False:
            ...
            self.class_graph = Graph(self.class_graph, 'class',
                                     interval_lines=True, x_range=(0, g.videos.video_length))
        ...
        self.end_line_edit.setText(str(end))
        self.end_line_edit.setValidator(QtGui.QIntValidator(0, g.videos.video_length))
    ...
    def verify_labels(self):
        ...
        for window in g.windows.windows:
            ...
            last_window_end = window_end
        if not (last_window_end == g.videos.video_length):
            failed_test.append(5)
            verified = False
